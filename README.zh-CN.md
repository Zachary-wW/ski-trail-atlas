...
产品长期设计并不限制在富龙。当前 Panorama Map 使用原创 SVG 和带证据来源的相对拓扑图，而不是重新发布来源雪道图；现阶段所有几何位置仍明确标记为未验证、不可用于导航。
...
- 一张原创全局 SVG 拓扑图，包含 33 条可交互 Trail Location 与主要 L1/L2/L3/L5/L7 索道；
- 每条雪道都有经过 Map Node 与证据边界校验的有向拓扑起终点；
- 已加入结构示意 Route Planner：串联已发布的下行雪道 edge 与上行索道 edge，高亮完整路线，并对不支持的连接明确返回无路线而不是猜测；
